...
f <- function(x1, x2, x_lwr, x_upr) {
  x1 <- recover_input(x1, x_lwr, x_upr)
  x2 <- recover_input(x2, x_lwr, x_upr)
...
```{r}
pipe <- function(ini_guess, acquisition_function, iteration, x_lwr, x_upr, kappa = 2) {
  ### Firstly map the initial guess into [0,1]
  ini_guess <- ini_guess %>% mutate(x1 = transform_input(x1, x_lwr, x_upr)) %>% mutate(x2 = transform_input(x2, x_lwr, x_upr))
  ...
  while(t < iteration) {
    ...
    evaluation[nrow(evaluation)+1,] <- c(x_next, f(x_next[1], x_next[2], x_lwr, x_upr), t+1)
    t <- t+1
  }
  ...
  evaluation <- evaluation %>% mutate(x1 = recover_input(x1, x_lwr, x_upr))
  ### Acquisition function's plot
  plot_1 <- acq_data %>% ggplot() +
    geom_contour_filled(mapping = aes(x = x1, y = x2, z = acq_value)) +
  ...
  plot_data <- data.frame(x1 = x1_plot, x2 = x2_plot, y = f(x1_plot, x2_plot))
  plot_2 <- evaluation %>% filter(iteration > 0) %>% 
    ggplot() +
    geom_contour_filled(data = plot_data,
                        mapping = aes(x = x1, y = x2, z = y), 
...
res <- pipe(ini_set, "Probability of improvement", 15)
```
