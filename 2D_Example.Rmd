...
f <- function(x1, x2) {
  ### Use linear function here, so the range is from [0,10]
  ### Can use logistic instead
  x1 = x1*10
  x2 = x2*10
...
pipe <- function(ini_guess, acquisition_function, iteration, kappa = 2) {
  ...
  ini_guess <- ini_guess %>% mutate(x1 = x1/10) %>% mutate(x2 = x2/10)
  ...
  while(t < iteration) {
    ...
    evaluation[nrow(evaluation)+1,] <- c(x_next, f(x_next[1], x_next[2]), t+1)
    t <- t+1
  }
  ...
  print(evaluation %>% filter(iteration > 0))
...
evaluation <- data.frame(x1 = c(1,2,3), x2 = c(3,4,5)) %>% mutate(y = f(x1, x2))
res <- pipe(evaluation, "Probability of improvement", 15)
```
