...
safe_results <- purrr::map2(our_grid$t, our_grid$my_seed,
...
```{r}
returned_sensitivity_results <- purrr::map(safe_results,
                                           function(ll){
                                             if(is.null(ll$error)){
                                               return(ll$result)
                                             }
                                           })
a <- purrr::map(returned_sensitivity_results,
                function(ll) {
                  if(is.null(ll)){
                    return(1)
                  }
...
```{r}
saveRDS(res, "res.rds")
saveRDS(safe_results, "safe_results.rds")
saveRDS(returned_sensitivity_results, "returned_sentivity_results.rds")
```
