...
gp_pred_mean_plot_2 <- function(pred_data, evaluation) {
  pred_data %>% 
    ...
    geom_point(data = evaluation, 
               mapping = aes(x = x1, y = x2), 
               shape = 0, color = "cyan") +
    geom_point(data = data.frame(x = c(1), y = c(1)), mapping = aes(x = x, y = y), color = "red") +
    coord_equal() +
    facet_wrap(~ iteration) +
...
```{r}
### Find the closest factor to sqrt(x) of x.
find_factor <- function(x) {
  factor = ceiling(sqrt(x))
  ### Always true when factor == x
  while(x %% factor != 0) {
    factor <- factor+1
  }
  return(factor)
...
```{r}
safe_results_Rag %>% length()
```

Extract the results when there was NOT an error.  

```{r}
returned_sensitivity_results_Rag_2 <- purrr::map(safe_results_Ham,
                                           function(ll){
                                             if(is.null(ll$error)){
                                               return(ll$result)
                                             }
                                           })
returned_sensitivity_results_Rag_2 <- purrr::map(returned_sensitivity_results_Rag_2,
                function(ll) {
                  if(!is.null(ll)){
...
returned_sensitivity_results_Ros_grid %>% 
  purrr::map_dfr(function(ll){ll}) %>% 
  group_by(random_seed, ini, acq, find) %>% 
  ungroup() %>% 
  ggplot(mapping = aes(x = ini, y = factor(ite, levels=c(seq(1,50))))) +
...
saveRDS(safe_results_Rag, "safe_results_Rag.rds")
saveRDS(returned_sensitivity_results_Rag, "returned_sentivity_results_Rag.rds")
```
