...
### this time use a simple LHS design as the initial design
...
### use LHS with 9 observation as the initial design
library(lhs)

set.seed(4321)
init_lhs <- optimumLHS(n = 9, k = 2, maxSweeps = 101, eps = 0.01) %>% 
  as.data.frame() %>% tibble::as_tibble() %>% 
...
init_lhs %>% 
  ggplot(mapping = aes(x = x, y = y)) +
  geom_point(size = 4) +
  coord_equal(xlim = c(-0.01, 1.01), ylim = c(-0.01, 1.01)) +
  scale_x_continuous(breaks = seq(0, 1, length.out = 10)) +
  scale_y_continuous(breaks = seq(0, 1, length.out = 10)) +
  theme_bw() +
  theme(panel.grid.minor = element_blank())

### need to define lower and upper bounds on the two inputs
init_lwr <- c(x = -4.75, y = -4.75)

init_upr <- c(x = 4.75, y = 4.75)

### back transform to the original input ranges
init_grid <- purrr::pmap_dfc(list(init_lhs,
                                  init_lwr,
                                  init_upr),
                             function(v, vlwr, vupr){
                               (vupr - vlwr) * v + vlwr
                             })
