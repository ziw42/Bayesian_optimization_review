...
# Rosenbrock function
f <- function(x_list, lwr, upr) {
  sum <- 0
  for(t in 1:(length(x_list)-1)) {
    x_list[t] <- recover_input(x_list[t], lwr, upr)
    sum <- sum + (100*(x_list[t+1]-x_list[t]^2)^2 + (1-x_list[t])^2)
  }
  return(sum)
}
...
pipe <- function(ini_guess, acquisition_function, iteration, lwr, upr, kappa = 2) {
  ...
  for(t in 1:(ncol(ini_guess)-1)) {
    ini_guess[t] = purrr::map(ini_guess[t], transform_input, lwr, upr)
  }
  ...
  ###############################################################
  run_acquisition <- function(Y_hat, MSE, y_min, kappa) {
  ...
  expand_grid <- function(dimension, length.out) {
    df_new <- data.frame()
    for(i in 1:dimension) {
      temp <- df_new
      for(t in 1:(length.out-1)) {
        df_new <- rbind(df_new, temp)
      }
      temp2 <- data.frame()
      for(t in seq(0, 1, length.out = length.out)) {
        temp2 <- rbind(temp2, matrix(rep(t,length.out^(i-1))))
      }
      if(i == 1) {
        df_new <- temp2
      }
      else {
        df_new <- cbind(df_new, temp2)
      }
    }
    
    return(df_new)
  }
  df_new <- expand_grid(ncol(ini_guess)-1, 101)
  
  ### Add corresponding iteration to the initial guess.
  ...
  for(t in 0:iteration) {
    ...
    browser()
    fit <- GP_fit(
     ### Excepte y and iteration.
     X = evaluation[c(-ncol(evaluation), -(ncol(evaluation)-1))] %>% as.matrix(),
     Y = evaluation$y,
    ...
    #    Y_hat <- (my_pred %>% as.data.frame() %>% filter(xnew.1 == x1, xnew.2 == x2, xnew.3 == x3)) $ Y_hat
    #    MSE <- (my_pred %>% as.data.frame() %>% filter(xnew.1 == x1, xnew.2 == x2, xnew.3 == x3)) $ MSE
    temp_df <- df_new %>%
      mutate(Y_hat = pred_from_gp[which(pred_from_gp$a == min(df$Amount)), ]) %>%
      mutate(acq_value = 
               run_acquisition(pred_from_gp, x1, x2, x3, y_min, kappa)) %>%
    ...
    evaluation[nrow(evaluation)+1,] <- c(x_next, f(x_next[1], x_next[2], x_next[3], lwr, upr), t+1)
  }
  ### Calculate the last iteration's data
  fit <- GP_fit(
    X = evaluation %>% select(x1, x2, x3) %>% as.matrix(),
    Y = evaluation$y,
    corr = list(type = "exponential", power = 2)
  )
  pred_from_gp <- predict.GP(fit, xnew = df_new)$complete_data %>%
  ...
  res <- list(x1 = evaluation$x1[res_index], x2 = evaluation$x2[res_index], x3 = evaluation$x3[res_index], y = evaluation$y[res_index])
...
complete_process <- function(ini_n, iteration, acquisition_function) {
  ...
  x_design <- maximinLHS(n = ini_n, k = 3) %>% as.matrix() %>% as.data.frame() %>%
