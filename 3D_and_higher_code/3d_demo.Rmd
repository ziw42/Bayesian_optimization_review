...
f <- function(x1, x2, x3, lwr, upr) {
  ...
  x2 <- recover_input(x2, lwr, upr)
  x3 <- recover_input(x3, lwr, upr)
  return(30 + (x1^2-10*cos(2*pi*x1)) + (x2^2-10*cos(2*pi*x2)) + (x3^2-10*cos(2*pi*x3)))
}
...
pipe <- function(ini_guess, acquisition_function, iteration, lwr, upr, kappa = 2) {
  ...
  run_acquisition <- function(my_pred, x1, x2, x3, y_min, kappa) {
...
find1_opt <- function(temp, x1_, x2_, x3_, sig, ite) {
  ...
  for(t in 1:nrow(temp)) {
    ### We tolerate error smaller than 0.1
...
save_acq_res <- function(acq_name, res, ini) {
  ...
  ### Paras stores the optimistic points' x1, x2, and x3 values.
  ### For Rosenbrock function, the known optimal value is (1,1,1)
  paras <- data.frame(x1 = c(1), x2 = c(1), x3 = c(1))
  ### Search one point in one loop.
  for(t in 1:nrow(paras)) {
    si = find1_opt(temp, paras[t,"x1"], paras[t,"x2"], paras[t,"x3"], sig, ite)
    sig = si[[1]]
    ite = si[[2]]
  }
...
run_for_one_init_size <- function(t, random_seed)
{
  ...
  res_1 <- complete_process(t, 30, "Expected improvement")
  set.seed(random_seed)
  res_2 <- complete_process(t, 30, "Probability of improvement")
  set.seed(random_seed)
  res_3 <- complete_process(t, 30, "Confidence lower bound")
  #browser()
